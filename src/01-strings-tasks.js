...
function encodeToRot13(str) {
  const alph = 'ABCDEFGHIJKLMNOPQRSTUVWXYZABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyzabcdefghijklmnopqrstuvwxyz';
  const arr = str.split('');
  const result = [];
  for (let i = 0; i < arr.length; i += 1) {
    if (arr[i] === ' ' || arr[i] === '!' || arr[i] === '?') {
      ...
      const index = alph.indexOf(arr[i]);
      const num = index + 13;
      result.push(alph[num]);
    }
  } return result.join('');
}
